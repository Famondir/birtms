...
ppmc_data_spm_1pl <- fit_1d_1pl_spm %>% birtms::get_ppmcdatasets(
  ppmcMethod = 'C', post_responses = posterior_responses_spm_1pl)
ppmc_data_spm_1pl_infit <- birtms::get_ppmccriteria(
  ppmcdata = ppmc_data_spm_1pl, ppmcMethod = 'C', criteria = 'infit')
birtms::plot_fit_statistic(model = fit_1d_1pl_spm, data = ppmc_data_spm_1pl_infit, 
                           units = c(1,12))
...
```{r}
ppmc_data_spm_1pl_infit %>% group_by(item) %>% select(crit) %>% birtms::custom_skim()
```
