...
# }
# 
# if (file.exists("../../models/gdcp/ppmcdata_c_1pl_spm_outfit.rds")) {
#   ppmc_data_spm_1pl_outfit <- readRDS("../../models/gdcp/ppmcdata_c_1pl_spm_outfit.rds")
# } else {
#   ppmc_data_spm_1pl_outfit <- birtms::get_ppmccriteria(model = fit_1d_1pl_spm,
#     ppmcdata = ppmc_data_spm_1pl, ppmcMethod = 'C', criteria = 'outfit')
#   ppmc_data_spm_1pl_outfit %>% saveRDS("../../models/gdcp/ppmcdata_c_1pl_spm_outfit.rds")
# }
...
#   ppmcMethod = 'C', post_responses = posterior_responses_spm_1pl)
# 
# ppmc_data_spm_1pl_outfit <- fit_1d_1pl_spm %>% birtms::get_ppmccriteria(
#   ppmcdata = ppmc_data_spm_1pl, ppmcMethod = 'C', criteria = 'outfit')

ppmc_data_spm_1pl_mixed <- fit_1d_1pl_spm %>% birtms::get_ppmcdatasets(
  ppmcMethod = 'M', crit = ll, post_responses = posterior_responses_spm_1pl)
...
```{r, include=FALSE}
outfit_skim_1pl <- ppmc_data_spm_1pl_outfit_mixed %>% group_by(item) %>% select(item, crit) %>% birtms::custom_skim()
```

```{r, echo=TRUE}
outfit_skim_1pl %>% select(item, numeric.mean, numeric.mode, numeric.hdi.lower, numeric.hdi.upper) %>% mutate_if(is.numeric, ~round(.x, 2))
```
...
```{r echo=FALSE}
# if (file.exists("../../models/gdcp/ppmcdata_c_2pl_spm.rds")) {
#   ppmc_data_spm_2pl <- readRDS("../../models/gdcp/ppmcdata_c_2pl_spm.rds")
# } else {
#   ppmc_data_spm_2pl <- fit_1d_2pl_spm %>% birtms::get_ppmcdatasets(
#     ppmcMethod = 'C', post_responses = posterior_responses_spm_2pl)
#   ppmc_data_spm_2pl %>% saveRDS("../../models/gdcp/ppmcdata_c_2pl_spm.rds")
# }
# 
# if (file.exists("../../models/gdcp/ppmcdata_c_2pl_spm_outfit.rds")) {
#   ppmc_data_spm_2pl_outfit <- readRDS("../../models/gdcp/ppmcdata_c_2pl_spm_outfit.rds")
# } else {
#   ppmc_data_spm_2pl_outfit <- birtms::get_ppmccriteria(model = fit_1d_2pl_spm,
#     ppmcdata = ppmc_data_spm_2pl, ppmcMethod = 'C', criteria = 'outfit')
#   ppmc_data_spm_2pl_outfit %>% saveRDS("../../models/gdcp/ppmcdata_c_2pl_spm_outfit.rds")
# }
...
ppmc_infit_data <- setNames(data.frame(
  ppmc_data_spm_1pl_outfit_mixed[,c(1,5)],ppmc_data_spm_2pl_outfit_mixed[,c(5)]), 
  c('item', '1pl', '2pl'))
...
<div style="float: right; width: 50%;">
<div class="lessspace">**Modellcheck mit birtms**</div>
